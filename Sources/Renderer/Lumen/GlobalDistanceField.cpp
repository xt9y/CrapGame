        ...
        ) 
{
    ...
    const float voxel_size =
        clipmap->extent / static_cast<float>(clipmap->resolution);
    ...
    for (int z = 0; z < clipmap->resolution; ++z) 
    {
        for (int y = 0; y < clipmap->resolution; ++y) 
        {
            ...
            {
                const Math::Vec3 position = {
                    clipmap->minimum.x + (static_cast<float>(x) + 0.5f) * voxel_size,
                    clipmap->minimum.y + (static_cast<float>(y) + 0.5f) * voxel_size,
                    clipmap->minimum.z + (static_cast<float>(z) + 0.5f) * voxel_size,
                };
        ...
        ) const 
{
    ...
    const float normalized_x = gdfClampExact(
                (position.x - clipmap.minimum.x) / clipmap.extent,
                0.0f,
                1.0f
            ),
            normalized_y = gdfClampExact(
                (position.y - clipmap.minimum.y) / clipmap.extent,
                0.0f,
                1.0f
            ),
            normalized_z = gdfClampExact(
                (position.z - clipmap.minimum.z) / clipmap.extent,
                0.0f,
                1.0f
            );

    const float grid_x = normalized_x *
                static_cast<float>(clipmap.resolution_minus_one),
                grid_y = normalized_y *
                static_cast<float>(clipmap.resolution_minus_one),
                grid_z = normalized_z *
                static_cast<float>(clipmap.resolution_minus_one);
    ...
    const int x0 = static_cast<int>(std::floor(grid_x)),
              ...
              x1 = std::min(clipmap.resolution_minus_one, x0 + 1),
              y1 = std::min(clipmap.resolution_minus_one, y0 + 1),
              z1 = std::min(clipmap.resolution_minus_one, z0 + 1);
