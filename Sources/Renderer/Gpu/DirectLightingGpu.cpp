...
bool DirectLightingGpu::uploadBuffer(GLuint buffer,std::size_t *capacity,const void *data,std::size_t size,std::string *error)
{
    ...
    if(size>*capacity){std::size_t nc=std::max<std::size_t>(256u,*capacity);while(nc<size)nc*=2u;GL15.glBufferData(GL_SHADER_STORAGE_BUFFER,static_cast<LWCGLsizeiptr>(nc),nullptr,GL_DYNAMIC_DRAW);*capacity=nc;}
