...
void main(){ivec2 pixel=ivec2(gl_GlobalInvocationID.xy),dimensions=imageSize(oViewSpecular);if(pixel.x>=dimensions.x||pixel.y>=dimensions.y)return;float depth=texelFetch(sDepth,pixel,0).r;if(!gbufferDepthValid(depth)||uStaticLightIndex<0){imageStore(oViewSpecular,pixel,vec4(0.0));return;}vec3 position=gbufferReconstructWorld(pixel,dimensions,depth,uGBufferInverseViewProjection);vec4 nr=texelFetch(sNormalRoughness,pixel,0),am=texelFetch(sAlbedoMetallic,pixel,0),si=texelFetch(sSpecularIor,pixel,0),adv=texelFetch(sAdvancedMaterial,pixel,0),ta=texelFetch(sTangentAnisotropy,pixel,0);LightData light=lights[uStaticLightIndex];if(int(light.positionType.w+0.5)!=0){imageStore(oViewSpecular,pixel,vec4(0.0));return;}vec3 normal=normalize(nr.xyz),albedo=am.xyz,ld=normalize(-light.directionRange.xyz),viewDirection=normalize(uCameraPosition-position);float nl=max(dot(normal,ld),0.0),metallic=clamp(am.w,0.0,1.0),roughness=nr.w;if(nl<=0.0){imageStore(oViewSpecular,pixel,vec4(0.0));return;}float visibility=(uStaticShadowEnabled!=0&&light.coneShadow.z>0.5)?staticShadowVisibility(position):1.0;vec3 radiance=light.colorIntensity.xyz*light.colorIntensity.w*visibility;vec3 full=evaluateMaterialPbr(albedo,metallic,roughness,si.rgb,si.a,adv.x,adv.y,adv.z,ta.a,ta.xyz,normal,viewDirection,ld,radiance);vec3 lambert=albedo*(1.0-metallic)/PI*radiance*nl;imageStore(oViewSpecular,pixel,vec4(full-lambert,1.0));}
)GLSL";
...
bool ViewSpecularGpu::init(std::string *error)
{
    ...
    camera_location_=GL20.glGetUniformLocation(program_,"uCameraPosition");static_light_index_location_=GL20.glGetUniformLocation(program_,"uStaticLightIndex");shadow_enabled_location_=GL20.glGetUniformLocation(program_,"uStaticShadowEnabled");shadow_matrix_location_=GL20.glGetUniformLocation(program_,"uStaticShadowViewProjection");inverse_view_projection_location_=GL20.glGetUniformLocation(program_,"uGBufferInverseViewProjection");
    if(camera_location_<0||static_light_index_location_<0||shadow_enabled_location_<0||shadow_matrix_location_<0||inverse_view_projection_location_<0){setError(error,"GPU view specular uniforms are unavailable");shutdown();return false;}if(error)error->clear();return true;
}
...
bool ViewSpecularGpu::render(const GBufferGpu& gbuffer,const StaticShadowCacheGpu& shadow,const Math::Vec3& camera,const DirtyTileGpu *dirty_tiles,std::string *error)
{
    ...
    GL20.glUseProgram(program_);GL20.glUniform3f(camera_location_,camera.x,camera.y,camera.z);GL20.glUniform1i(static_light_index_location_,static_light_index_);GL20.glUniform1i(shadow_enabled_location_,shadow.enabled()&&static_light_index_>=0?1:0);GL20.glUniformMatrix4fv(shadow_matrix_location_,1,GL_FALSE,shadow.lightViewProjection().value);GL20.glUniformMatrix4fv(inverse_view_projection_location_,1,GL_FALSE,gbuffer.inverseViewProjection().value);GL30.glBindBufferBase(GL_SHADER_STORAGE_BUFFER,8,light_buffer_);GL42.glBindImageTexture(0,texture_,0,GL_FALSE,0,GL_WRITE_ONLY,GL_RGBA16F);GL43.glDispatchCompute(static_cast<GLuint>((width_+7)/8),static_cast<GLuint>((height_+7)/8),1);GL42.glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT|GL_TEXTURE_FETCH_BARRIER_BIT);GL20.glUseProgram(0);GL30.glBindBufferBase(GL_SHADER_STORAGE_BUFFER,8,0);for(GLuint unit=0;unit<=6;++unit)bindTexture(unit,0);GLModern.glActiveTexture(GL_TEXTURE0);if(error)error->clear();return true;
}

void ViewSpecularGpu::shutdown()
{
    if(texture_!=0)glDeleteTextures(texture_);texture_=0;destroyProgram(&program_);light_buffer_=0;camera_location_=static_light_index_location_=shadow_enabled_location_=shadow_matrix_location_=-1;inverse_view_projection_location_=-1;static_light_index_=-1;width_=0;height_=0;
}
