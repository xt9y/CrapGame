...
public:
    ...
    bool updateScene(const Ecs::World& world,std::string* error=nullptr)
    {
        ...
        for(const Ecs::Entity entity:world.entities())
        {
            ...
            if(transform && light && light->intensity>0.0f)
            {
                ...
                gpu.position_type[3]=light->type==Ecs::LightType::Directional?0.0f:(light->type==Ecs::LightType::Point?1.0f:2.0f);
            ...
            // The shared direct-light/Lumen acceleration structure is still an
            // analytic Cube/Plane representation. Imported triangles are
            // rasterized by GBufferGpu and must not be misrepresented here.
            if(mesh->loaded_mesh!=Ecs::INVALID_ASSET_HANDLE) continue;
        ...
        if(!bench_reported_ && (config.stress_primitives>0u || config.mode!=BvhMode::Auto))
        {
            std::fprintf(stderr,"GPU BVH benchmark: scene %zu + stress %zu = %zu primitives, mode %s, traversal %s\n",scene_primitive_count,config.stress_primitives,primitives_.size(),bvhModeName(config.mode),use_bvh_?"bvh":"linear");
            bench_reported_=true;
        }

        if(!ensureBvhShader(use_bvh_ || config.stress_primitives>0u,error)) return false;
        ...
        if(use_bvh_)
        {
            if(!ensureBvhBuffer(error)) return false;
            const bool topology_matches=!bvh_nodes_.empty() && bvh_primitive_count_==primitives_.size();
            bool refit_ok=false;
            if(topology_matches) refit_ok=refitBvh(&bvh_nodes_,primitive_bounds_);
            if(!refit_ok)
            {
            ...
            if(!uploadChangedRecords(bvh_node_buffer_,&bvh_node_capacity_,bvh_nodes_,&uploaded_bvh_nodes_,error)) return false;
        }
        else
        {
            bvh_nodes_.clear();
    ...
    bool dispatch(const GBufferGpu& gbuffer,const Math::Vec3& camera_position,std::string* error=nullptr)
    {
        ...
        if(bvh_program_active_)
        {
            ...
            if(use_bvh) GL30.glBindBufferBase(GL_SHADER_STORAGE_BUFFER,7,bvh_node_buffer_);
        }
...
private:
    ...
    {
        if(!bench_config_initialized_){ bench_config_=bvhBenchConfig(); bench_config_initialized_=true; }
    ...
    void appendStressPrimitives(std::size_t count)
    {
        ...
        {
            const std::size_t grid_x=index%side, grid_z=index/side;
            Ecs::TransformComponent transform={};
            transform.position={(static_cast<float>(grid_x)-half)*1.20f,0.55f+static_cast<float>(index%3u)*0.18f,(static_cast<float>(grid_z)-half)*1.20f-3.0f};
            transform.rotation={static_cast<float>((index*13u)%31u),static_cast<float>((index*29u)%360u),0.0f};
            transform.scale={0.32f,0.32f,0.32f};
            PrimitiveGpu primitive={};
            primitive.position_type[0]=transform.position.x; primitive.position_type[1]=transform.position.y; primitive.position_type[2]=transform.position.z; primitive.position_type[3]=0.0f;
            primitive.rotation[0]=transform.rotation.x; primitive.rotation[1]=transform.rotation.y; primitive.rotation[2]=transform.rotation.z;
            primitive.scale[0]=transform.scale.x; primitive.scale[1]=transform.scale.y; primitive.scale[2]=transform.scale.z; primitive.scale[3]=0.0f;
            primitive.albedo_metallic[0]=0.35f+static_cast<float>(index%5u)*0.07f; primitive.albedo_metallic[1]=0.42f; primitive.albedo_metallic[2]=0.55f; primitive.albedo_metallic[3]=index%4u==0u?0.55f:0.0f; primitive.emissive_roughness[3]=0.45f;
            const std::uint32_t primitive_index=static_cast<std::uint32_t>(primitives_.size());
            primitives_.push_back(primitive);
            primitive_bounds_.push_back(primitiveBounds(transform,Ecs::MeshType::Cube,primitive_index));
        }
    }
    ...
    bool ensureBvhShader(bool activate,std::string* error)
    {
        ...
        {
            constexpr const char* old_layout="layout(rgba16f,binding=2) readonly uniform image2D gAlbedoMetallic;";
            constexpr const char* new_layout="layout(rgba8,binding=2) readonly uniform image2D gAlbedoMetallic;";
            const std::size_t pos=shader_source.find(old_layout);
            if(pos==std::string::npos){ setInlineError(error,"GPU BVH direct-light material image layout is unavailable"); return false; }
            shader_source.replace(pos,std::char_traits<char>::length(old_layout),new_layout);
        }
        GLuint candidate=createComputeProgram(shader_source.c_str(),error);
        if(candidate==0) return false;
        GLint camera=-1,lights=-1,primitives=-1,nodes=-1;
        if(!queryBvhLocations(candidate,&camera,&lights,&primitives,&nodes)){ destroyProgram(&candidate); setInlineError(error,"GPU BVH direct-light uniforms are unavailable"); return false; }
        bvh_shader_validated_=true;
        if(!activate){ destroyProgram(&candidate); if(error) error->clear(); return true; }
        destroyProgram(&program_);
        program_=candidate; camera_location_=camera; light_count_location_=lights; primitive_count_location_=primitives; bvh_node_count_location_=nodes; bvh_program_active_=true;
        if(error) error->clear();
        return true;
    }
    ...
    bool ensureBvhBuffer(std::string* error)
    {
        ...
        if(bvh_node_buffer_==0){ setInlineError(error,"failed to allocate shared GPU BVH buffer"); return false; }
        return true;
    }
    ...
    {
        if(!capacity || !uploaded){ setInlineError(error,"invalid GPU dirty-range destination"); return false; }
        if(current.empty()){ if(*capacity==0 && !uploadBuffer(buffer,capacity,nullptr,0,error)) return false; uploaded->clear(); return true; }
        const std::size_t required=current.size()*sizeof(T);
        if(*capacity==0 || required>*capacity || current.size()!=uploaded->size()){ if(!uploadBuffer(buffer,capacity,current.data(),required,error)) return false; *uploaded=current; return true; }
        GL15.glBindBuffer(GL_SHADER_STORAGE_BUFFER,buffer);
        forEachDirtyRange(current,*uploaded,[&](std::size_t first,std::size_t count){ GL15.glBufferSubData(GL_SHADER_STORAGE_BUFFER,static_cast<LWCGLintptr>(first*sizeof(T)),static_cast<LWCGLsizeiptr>(count*sizeof(T)),current.data()+first); });
        *uploaded=current;
        return true;
    }

    static Math::Vec3 toVec3Inline(const Ecs::Vec3& v){ return {v.x,v.y,v.z}; }
    static Math::Vec3 lightForwardInline(const Ecs::TransformComponent& transform){ return Math::normalize(Math::transformDirection(Math::rotationEuler(toVec3Inline(transform.rotation)),{0.0f,0.0f,-1.0f})); }
    static GLenum imageFormatInline(SurfaceFormat format){ return format==SurfaceFormat::Rgba8?GL_RGBA8:GL_RGBA16F; }
    ...
    bool bench_config_initialized_=false, bench_reported_=false, use_bvh_=false, bvh_shader_validated_=false, bvh_program_active_=false;
