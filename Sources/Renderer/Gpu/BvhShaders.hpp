#ifndef CRAPGAME_RENDERER_GPU_BVHSHADERS_HPP
#define CRAPGAME_RENDERER_GPU_BVHSHADERS_HPP
...
constexpr const char *DIRECT_LIGHTING_BVH_COMPUTE = R"GLSL(
#version 430 core
...
layout(std430, binding = 8) readonly buffer BvhNodeBuffer { BvhNode bvhNodes[]; };
layout(std430, binding = 9) readonly buffer BvhIndexBuffer { uint bvhPrimitiveIndices[]; };
...
bool intersectBox(vec3 ro,vec3 rd,PrimitiveData p,float maximumDistance){vec3 s=p.scale.xyz;vec3 q=vec3(abs(s.x)>EPSILON?s.x:0.00001,abs(s.y)>EPSILON?s.y:0.00001,abs(s.z)>EPSILON?s.z:0.00001);vec3 o=inverseRotate(ro-p.positionType.xyz,p.rotation.xyz)/q;vec3 d=inverseRotate(rd,p.rotation.xyz)/q;vec3 inv=vec3(abs(d.x)>EPSILON?1.0/d.x:1e20,abs(d.y)>EPSILON?1.0/d.y:1e20,abs(d.z)>EPSILON?1.0/d.z:1e20);vec3 t0=(vec3(-0.75)-o)*inv,t1=(vec3(0.75)-o)*inv;vec3 mn=min(t0,t1),mx=max(t0,t1);float tn=max(max(mn.x,mn.y),mn.z),tf=min(min(mx.x,mx.y),mx.z);if(tf<max(tn,SHADOW_BIAS))return false;float t=tn>SHADOW_BIAS?tn:tf;return t>SHADOW_BIAS&&t<maximumDistance;}
bool intersectPlane(vec3 ro,vec3 rd,PrimitiveData p,float maximumDistance){vec3 s=p.scale.xyz;vec3 q=vec3(abs(s.x)>EPSILON?s.x:0.00001,abs(s.y)>EPSILON?s.y:0.00001,abs(s.z)>EPSILON?s.z:0.00001);vec3 o=inverseRotate(ro-p.positionType.xyz,p.rotation.xyz)/q;vec3 d=inverseRotate(rd,p.rotation.xyz)/q;if(abs(d.y)<=EPSILON)return false;float t=-o.y/d.y;if(t<=SHADOW_BIAS||t>=maximumDistance)return false;vec3 h=o+d*t;return abs(h.x)<=0.5&&abs(h.z)<=0.5;}
bool intersectAabb(vec3 ro,vec3 rd,vec3 mn,vec3 mx,float maximumDistance){vec3 inv=vec3(abs(rd.x)>EPSILON?1.0/rd.x:1e20,abs(rd.y)>EPSILON?1.0/rd.y:1e20,abs(rd.z)>EPSILON?1.0/rd.z:1e20);vec3 t0=(mn-ro)*inv,t1=(mx-ro)*inv;vec3 lo=min(t0,t1),hi=max(t0,t1);float tn=max(max(lo.x,lo.y),lo.z),tf=min(min(hi.x,hi.y),hi.z);return tf>=max(tn,0.0)&&tn<maximumDistance;}
bool primitiveHit(int index,vec3 origin,vec3 direction,float maximumDistance){PrimitiveData p=primitives[index];return int(p.positionType.w+0.5)==0?intersectBox(origin,direction,p,maximumDistance):intersectPlane(origin,direction,p,maximumDistance);}
bool shadowed(vec3 position,vec3 normal,vec3 direction,float maximumDistance){vec3 origin=position+normalize(normal)*SHADOW_BIAS*2.0;if(uBvhNodeCount<=0){for(int i=0;i<uPrimitiveCount;++i)if(primitiveHit(i,origin,direction,maximumDistance))return true;return false;}int stack[64];int stackSize=1;stack[0]=0;while(stackSize>0){int ni=stack[--stackSize];if(ni<0||ni>=uBvhNodeCount)continue;BvhNode n=bvhNodes[ni];if(!intersectAabb(origin,direction,n.boundsMinimum.xyz,n.boundsMaximum.xyz,maximumDistance))continue;if(n.meta.w>0){for(int j=0;j<n.meta.w;++j){int pi=int(bvhPrimitiveIndices[n.meta.z+j]);if(pi>=0&&pi<uPrimitiveCount&&primitiveHit(pi,origin,direction,maximumDistance))return true;}}else if(stackSize<=61){stack[stackSize++]=n.meta.x;stack[stackSize++]=n.meta.y;}}return false;}
...
vec3 evaluatePbr(vec3 albedo,float metallic,float roughness,vec3 normal,vec3 viewDirection,vec3 lightDirection,vec3 radiance){vec3 n=normalize(normal),v=normalize(viewDirection),l=normalize(lightDirection),h=normalize(v+l);float nl=clamp(dot(n,l),0.0,1.0),nv=clamp(dot(n,v),0.0,1.0);if(nl<=0.0||nv<=0.0)return vec3(0.0);vec3 f0=mix(vec3(0.04),albedo,clamp(metallic,0.0,1.0)),f=fresnelSchlick(dot(h,v),f0);float r=clamp(roughness,0.04,1.0),d=distributionGgx(n,h,r),g=geometrySmith(n,v,l,r);vec3 spec=f*d*g/(4.0*nv*nl+EPSILON),dw=(vec3(1.0)-f)*(1.0-clamp(metallic,0.0,1.0));return (dw*(albedo/PI)+spec)*radiance*nl;}
void main(){ivec2 pixel=ivec2(gl_GlobalInvocationID.xy),dimensions=imageSize(gPositionDepth);if(pixel.x>=dimensions.x||pixel.y>=dimensions.y)return;vec4 pd=imageLoad(gPositionDepth,pixel);if(pd.w<=0.0){imageStore(oDirect,pixel,vec4(0.055,0.070,0.105,1.0));return;}vec4 nr=imageLoad(gNormalRoughness,pixel),am=imageLoad(gAlbedoMetallic,pixel);vec3 emissive=imageLoad(gEmissive,pixel).xyz,position=pd.xyz,normal=normalize(nr.xyz),albedo=am.xyz,viewDirection=normalize(uCameraPosition-position);float roughness=nr.w,metallic=am.w;vec3 direct=emissive;for(int i=0;i<uLightCount;++i){LightData light=lights[i];int type=int(light.positionType.w+0.5);vec3 ld=vec3(0.0),radiance=vec3(0.0);float maxD=10000.0;if(type==0){ld=normalize(-light.directionRange.xyz);radiance=light.colorIntensity.xyz*light.colorIntensity.w;}else{vec3 toLight=light.positionType.xyz-position;float d=length(toLight);if(d<=EPSILON)continue;float attenuation=pointAttenuation(d,light.directionRange.w);if(attenuation<=0.0)continue;ld=toLight/d;maxD=max(SHADOW_BIAS,d-SHADOW_BIAS*2.0);float cone=1.0;if(type==2){vec3 fromLight=normalize(position-light.positionType.xyz);float cv=dot(normalize(light.directionRange.xyz),fromLight),inner=light.coneShadow.x,outer=light.coneShadow.y;cone=inner<=outer+EPSILON?(cv>=outer?1.0:0.0):clamp((cv-outer)/(inner-outer),0.0,1.0);}if(cone<=0.0)continue;radiance=light.colorIntensity.xyz*light.colorIntensity.w*attenuation*cone;}if(dot(radiance,radiance)<=0.0)continue;if(light.coneShadow.z>0.5&&shadowed(position,normal,ld,maxD))continue;direct+=evaluatePbr(albedo,metallic,roughness,normal,viewDirection,ld,radiance);}imageStore(oDirect,pixel,vec4(direct,1.0));}
)GLSL";

constexpr const char *LUMEN_TRACE_BVH_COMPUTE = R"GLSL(
#version 430 core
layout(local_size_x = 8, local_size_y = 8, local_size_z = 1) in;
...
bool intersectBox(vec3 ro,vec3 rd,PrimitiveData p,float maximumDistance,out float hitDistance,out vec3 hitNormal){vec3 s=safeScale(p.scale.xyz);vec3 o=inverseRotate(ro-p.positionType.xyz,p.rotation.xyz)/s,d=inverseRotate(rd,p.rotation.xyz)/s;vec3 inv=vec3(abs(d.x)>EPSILON?1.0/d.x:1e20,abs(d.y)>EPSILON?1.0/d.y:1e20,abs(d.z)>EPSILON?1.0/d.z:1e20);vec3 t0=(vec3(-0.75)-o)*inv,t1=(vec3(0.75)-o)*inv,mn=min(t0,t1),mx=max(t0,t1);float tn=max(max(mn.x,mn.y),mn.z),tf=min(min(mx.x,mx.y),mx.z);if(tf<max(tn,TRACE_BIAS))return false;hitDistance=tn>TRACE_BIAS?tn:tf;if(hitDistance<=TRACE_BIAS||hitDistance>=maximumDistance)return false;vec3 h=o+d*hitDistance,m=abs(h),ln;m.x>=m.y&&m.x>=m.z?ln=vec3(sign(h.x),0,0):m.y>=m.z?ln=vec3(0,sign(h.y),0):ln=vec3(0,0,sign(h.z));hitNormal=normalize(forwardRotate(ln/s,p.rotation.xyz));return true;}
bool intersectPlane(vec3 ro,vec3 rd,PrimitiveData p,float maximumDistance,out float hitDistance,out vec3 hitNormal){vec3 s=safeScale(p.scale.xyz);vec3 o=inverseRotate(ro-p.positionType.xyz,p.rotation.xyz)/s,d=inverseRotate(rd,p.rotation.xyz)/s;if(abs(d.y)<=EPSILON)return false;hitDistance=-o.y/d.y;if(hitDistance<=TRACE_BIAS||hitDistance>=maximumDistance)return false;vec3 h=o+d*hitDistance;if(abs(h.x)>0.5||abs(h.z)>0.5)return false;vec3 ln=d.y<0.0?vec3(0,1,0):vec3(0,-1,0);hitNormal=normalize(forwardRotate(ln/s,p.rotation.xyz));return true;}
bool intersectAabb(vec3 ro,vec3 rd,vec3 mn,vec3 mx,float maximumDistance){vec3 inv=vec3(abs(rd.x)>EPSILON?1.0/rd.x:1e20,abs(rd.y)>EPSILON?1.0/rd.y:1e20,abs(rd.z)>EPSILON?1.0/rd.z:1e20);vec3 t0=(mn-ro)*inv,t1=(mx-ro)*inv,lo=min(t0,t1),hi=max(t0,t1);float tn=max(max(lo.x,lo.y),lo.z),tf=min(min(hi.x,hi.y),hi.z);return tf>=max(tn,0.0)&&tn<maximumDistance;}
void testPrimitive(int index,vec3 ro,vec3 rd,inout int hitIndex,inout float hitDistance,inout vec3 hitNormal){PrimitiveData p=primitives[index];float candidate=hitDistance;vec3 normal=vec3(0);bool hit=int(p.positionType.w+0.5)==0?intersectBox(ro,rd,p,hitDistance,candidate,normal):intersectPlane(ro,rd,p,hitDistance,candidate,normal);if(hit&&candidate<hitDistance){hitIndex=index;hitDistance=candidate;hitNormal=normal;}}
bool traceScene(vec3 ro,vec3 rd,float maximumDistance,out int hitIndex,out float hitDistance,out vec3 hitNormal){hitIndex=-1;hitDistance=maximumDistance;hitNormal=vec3(0);if(uBvhNodeCount<=0){for(int i=0;i<uPrimitiveCount;++i)testPrimitive(i,ro,rd,hitIndex,hitDistance,hitNormal);return hitIndex>=0;}int stack[64];int stackSize=1;stack[0]=0;while(stackSize>0){int ni=stack[--stackSize];if(ni<0||ni>=uBvhNodeCount)continue;BvhNode n=bvhNodes[ni];if(!intersectAabb(ro,rd,n.boundsMinimum.xyz,n.boundsMaximum.xyz,hitDistance))continue;if(n.meta.w>0){for(int j=0;j<n.meta.w;++j){int pi=int(bvhPrimitiveIndices[n.meta.z+j]);if(pi>=0&&pi<uPrimitiveCount)testPrimitive(pi,ro,rd,hitIndex,hitDistance,hitNormal);}}else if(stackSize<=61){stack[stackSize++]=n.meta.x;stack[stackSize++]=n.meta.y;}}return hitIndex>=0;}
