        ...
        ) 
{
    const Math::Vec3 edge_a = 
        Math::subtract(triangle.b, triangle.a);

    const Math::Vec3 edge_b = 
        Math::subtract(triangle.c, triangle.a);

    const Math::Vec3 p = 
        Math::cross(direction, edge_b);
    ...
    const Math::Vec3 t = 
        Math::subtract(origin, triangle.a);
...
void Scene::build (const Ecs::World& world) 
{
    ...
    for (const Ecs::Entity entity : world.entities()) 
    {
        ...
        for (std::size_t index = 0; 
                index + 2u < mesh_data.indices.size(); 
                index += 3u) 
        {
            const Mesh::Vertex& vertex_a = 
                mesh_data.vertices[mesh_data.indices[index + 0u]];

            const Mesh::Vertex& vertex_b = 
                mesh_data.vertices[mesh_data.indices[index + 1u]];

            const Mesh::Vertex& vertex_c = 
                mesh_data.vertices[mesh_data.indices[index + 2u]];
        ...
        ) const 
{
    ...
    const Math::Vec3 ray_direction = 
        Math::normalize(light_sample.direction);
    ...
    {
        maximum_distance = 10000.0f;
    }
    else 
    {
        maximum_distance = std::max(
                SHADOW_BIAS,
                maximum_distance - SHADOW_BIAS * 2.0f
            );
    }
